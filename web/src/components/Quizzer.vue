...
<template>
  ...
  <div class="quiz-wrapper" v-if="!questions">
    <form @submit.prevent="handleSubmit" class="form">
      <div class="form-group-col">
        ...
        <textarea id="quiz-name" name="quiz-name" required v-model="topic" placeholder="Choose any topic to generate"
          :disabled="submitting" />
      </div>

      <div class="form-group">
        <label for="quiz-description">Number of Questions:</label>
        <input type="number" max="4" min="1" id="quiz-description" name="quiz-description" v-model.number="question"
          :disabled="submitting" />
      </div>

      <div class="form-group">
        <label for="quiz-answers">Number of Answers:</label>
        <input type="number" min="1" max="4" id="quiz-answers" name="quiz-answers" v-model.number="answer"
          :disabled="submitting" />
      </div>
  ...
  <div class="start-test" v-if="questions">
    <div class="header">
      <h2>Questions</h2>
      <div class="actions">
        <a href="#" @click.prevent="tryAgain">Try again</a>&nbsp; | &nbsp;<a href="#"
          @click.prevent="handleRegenerate">Generate a new one</a>
      </div>
    </div>
    <h3>Topic: {{ topic }}</h3>
